...
def create_voting_dict(strlist):
    ...
    for s in strlist:
        s = s.strip().split(' ')
        voting_dict[s[0]] = [int(vote) for vote in s[4:]]
    return voting_dict

print('1.', create_voting_dict(dat))
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    """
    sen_a_vec = voting_dict[sen_a]
    sen_b_vec = voting_dict[sen_b]

    return sum([sen_a_vec[k]*sen_b_vec[k] for k in range(len(sen_a_vec))])

print('2.', 'Policy comparison: McCain vs. Obama =', \
      policy_compare('McCain', 'Obama', create_voting_dict(dat)))
...
def most_similar(sen, voting_dict):
    ...
    """
    most_similar = None
    highest_dot_product = -100
    for senator in voting_dict.keys():
        if senator == sen:
            continue
        dot_product = policy_compare(sen, senator, voting_dict)
        if dot_product > highest_dot_product:
            highest_dot_product = dot_product
            most_similar = senator
    return most_similar

print('3.', 'Most similar to Obama:', most_similar('Obama', create_voting_dict(dat)))
...
def least_similar(sen, voting_dict):
    ...
    """
    least_similar = None
    lowest_dot_product = 100
    for senator in voting_dict.keys():
        dot_product = policy_compare(sen, senator, voting_dict)
        if dot_product < lowest_dot_product:
            lowest_dot_product = dot_product
            least_similar = senator
    return least_similar

print('4.', 'Least similar to Obama:', least_similar('Obama', create_voting_dict(dat)))


## 5: (Task 2.12.5) Chafee, Santorum
most_like_chafee    = most_similar('Chafee', create_voting_dict(dat))
least_like_santorum = least_similar('Santorum', create_voting_dict(dat))

print('5a. Most like Chafee ->', most_like_chafee)
print('5b. Least like Santorum ->', least_like_santorum)

## 6: (Task 2.12.7) Most Average Democrat
def senator_set(strlist):
    dem = set()
    gop = set()
    for s in strlist:
        s = s.strip().split(' ')
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    for other_sen in sen_set:
        similarity_dict[other_sen] = policy_compare(sen, other_sen, voting_dict)
    similarity_sum = 0
    for other_sen in similarity_dict.keys():
        similarity_sum += similarity_dict[other_sen]
    return similarity_sum/len(similarity_dict.keys())

most_average_Democrat = None
highest_similarity_score = 0

for sen in dem:
    similarity_score = find_average_similarity(sen, dem, create_voting_dict(dat))
    if similarity_score > highest_similarity_score:
        highest_similarity_score = similarity_score
        most_average_Democrat = sen

print('6. Most average Democrat:', most_average_Democrat)
...
def find_average_record(sen_set, voting_dict):
    ...
    Example:
        ...
        [0.0, 1.0, 1.0]
    """
    average_record_vector = []
    for sen in sen_set:
        vote_sum = 0
        vote_num = 0
        for vote in voting_dict[sen]:
            vote_sum += vote
            vote_num += 1
        average_record_vector.append(vote_sum/vote_num)
    return average_record_vector

average_Democrat_record = find_average_record(dem, create_voting_dict(dat))
average_GOP_record = find_average_record(gop, create_voting_dict(dat))

print('7a. Average Democrat record:', average_Democrat_record)
print('7b. Average GOP record:', average_GOP_record)
...
def bitter_rivals(voting_dict):
    ...
    """
    bitter_rivals = None
    lowest_similarity_score = 1000
    senators = voting_dict.keys()
    for sen_a in senators:
        for sen_b in senators:
            similarity_score = policy_compare(sen_a, sen_b, voting_dict)
            if similarity_score < lowest_similarity_score:
                lowest_similarity_score = similarity_score
                bitter_rivals = (sen_a, sen_b)
    return bitter_rivals

print('8. Bitter rivals:', bitter_rivals(create_voting_dict(dat)))
